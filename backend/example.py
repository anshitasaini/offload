...
@app.post("/index-note/", tags=["note"])
async def index_note(indexNoteRequest: IndexNoteIn):
    from langchain.vectorstores import Pinecone
    from langchain.embeddings import OpenAIEmbeddings
    from langchain.text_splitter import MarkdownTextSplitter
    import pinecone
 
    pinecone.create_index("note-index", dimension=1536, metric="cosine")
 
    doc = indexNoteRequest.content
 
    embeddings = OpenAIEmbeddings()
    note_search = Pinecone.from_documents([doc], embeddings, index_name="note-index")